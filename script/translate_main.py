"""Ollama TXT 번역과 Prompt Preset 작성을 위한 콘솔 진입점입니다."""
...
def run_interactive_translation(
    ...
    loader = FileLoader(settings.input_dir)
    writer = FileWriter(settings.output_dir)
    input_files = loader.list_txt_files()
    writer.ensure_output_dir()
    if not input_files:
        _display_empty_input(output_func)
        return

    detected = LanguageDetector().detect_files(input_files)
...
def _display_translation_header(
    ...
    output_func("Local LLM TXT Translator")
    output_func("")
...
def _display_empty_input(output_func: OutputFunction) -> None:
    output_func("번역할 TXT 파일이 없습니다.")
    output_func("setting/input_txt 폴더에 파일을 추가해 주세요.")
